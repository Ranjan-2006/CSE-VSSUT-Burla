...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    projContainer.innerHTML = '<div style="text-align: center; padding: 40px;"><i class="fas fa-spinner fa-spin" style="font-size: 2rem; color: var(--blue-600);"></i><p style="margin-top: 10px;">Loading projects...</p></div>';

    try {
        const { data, error } = await supabase
            .from('research_and_project')
            .select('*')
            .eq('category', 'sponsored_project')
        ...
        if (!data || data.length === 0) {
            projContainer.innerHTML = `
                <div class="empty-state" style="text-align:center; padding: 40px;">
                    <i class="fas fa-project-diagram" style="font-size: 3rem; color: var(--gray-300); margin-bottom: 20px;"></i>
                    <p>No sponsored project records found.</p>
                </div>
            `;
            return;
        }
        ...
        let html = '<div class="projects-wrapper" style="background: var(--white); padding: 40px; border-radius: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.05);">';
        ...
        projContainer.innerHTML = '<div style="text-align: center; color: var(--red-500); padding: 40px;">Error loading projects.</div>';
    }
});
